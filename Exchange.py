...
from msgClasses import Exch_msg
...
class Orderbook_half:
        ...
        def book_CAN(self, time, order, pool_id, verbose):
                ...
                def add_tapeitem(eventlist, pool_id, time, oid, otype, qty, verbose):
                        ...
                        tape_event = {'pool_id': pool_id, 'type': 'CAN', 'time': time, 'oid': oid, 'otype': otype,
                                      'o_qty': qty}
        ...
        def book_take(self, time, order, pool_id, verbose):
                ...
                while good_price and (qty_remaining > 0) and (len(self.orders) > 0):
                        ...
                        if verbose:
                                print('BK_TAKE: qty_rem=%d; lob=%s; good_price=%s' % (
                                qty_remaining, str(self.lob), good_price))
                                sys.stdout.flush()
                        ...
                        if verbose: print(
                                        'BK_TAKE: best_lob _price=%d _order=%s qty=%d oid_from=%d oid_to=%d tid_from=%s tid_to=%s\n' %
                                        (best_lob_price, best_lob_order, best_lob_order_qty, oid_from, oid_to, tid_from,
                                         tid_to))
                        ...
                        if best_lob_order_qty >= qty_remaining:
                                ...
                                if best_lob_order_qty > 0:
                                        ...
                                        add_msg(msg_list, best_lob_order_tid, best_lob_order_oid, "FILL", [transaction],
                                                None, fee, verbose)
                                        # add_tapeitem(tape_events, 'Trade', time, price, qty, tid_from, tid_to, verbose)
                                ...
                                # add a message to the list of outgoing messages from exch to traders
                                add_msg(msg_list, best_lob_order_tid, best_lob_order_oid, "FILL", [transaction], None,
                                        fee, verbose)
...
class Orderbook(Orderbook_half):
        ...
        def process_order_LIM(self, time, order, verbose):
                ...
                if order.otype == 'Bid':
                        if len(self.asks.lob) > 0 and oprice >= self.asks.lob[0][0]:
                                ...
                                if verbose: print("Bid LIM $%s lifts best ask ($%s) =>IOC" % (
                                oprice, self.asks.lob[0][0]))
                                order.ostyle = 'IOC'
                        ...
                        if len(self.bids.lob) > 0 and oprice <= self.bids.lob[0][0]:
                                ...
                                if verbose: print(
                                                "Ask LIM $%s hits best bid ($%s) =>IOC" % (oprice, self.bids.lob[0][0]))
                                order.ostyle = 'IOC'
                                response = self.process_order_take(time, order, verbose)
                        else:
                                response = process_LIM(order, verbose)
        ...
        def process_order_pending(self, time, order, verbose):
                ...
                if order.ostyle == 'LOO' or order.ostyle == 'MOO':
                        ...
                        sys.exit('process_order_pending() given something other than LOO MOO LOC MOC')
...
class Exchange(Orderbook):
        ...
        def __init__(self, eid):
                ...
                self.drk = Orderbook(
                        eid + "Drk")  # NB just a placeholder -- in this version of BSE the dark pool is undefined
        ...
        def tape_update(self, tr, verbose):
                ...
                # updates the tape
                if verbose: print(
                                "Tape update: tr=%s; len(tape)=%d tape[-3:]=%s" % (tr, len(self.tape), self.tape[-3:]))
        ...
        def process_order(self, time, order, verbose, transactions):
                ...
                if response != None:
                        ...
                        for tape_event in tape_events:
                                if tape_event['type'] == 'Trade':
                                        transactions.write(
                                                '%s, %s, %s, %s, %s\n' % (
                                                        tape_event['party1'], tape_event['party2'], tape_event['time'],
                                                        tape_event['price'],
                                                        tape_event['qty']))
                        ...
                        for msg in trader_msgs:
                                if msg.tid == trader_id:
                                        total_fees += msg.fee
                                        if verbose: print('Trader %s adding fee %d from msg %s' % (
                                        trader_id, msg.fee, msg))
                        self.trader_recs[trader_id].balance += total_fees
                        if verbose: print('Trader %s Exch %s: updated balance=%d' % (
                        ...
                        if len(tape_events) > 0:
                                ...
                                for event in tape_events:
                                        if event['type'] == 'Trade':
                                                ...
                                                total_qty += event['qty']
                                                if verbose: print(
                                                                'total_cost=%d; total_qty=%d' % (total_cost, total_qty))
                                if total_qty > 0:
